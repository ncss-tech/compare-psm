...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, purl = FALSE,
                      fig.align = 'center', fig.path = './figs/compare_maps_local/')
...
Because the full tile is so large, here we can choose to test on a smaller area, e.g., 0.1 x 0.1 degrees, $\approx 11 \times 11$ km.

Set the size and offset from the lower-right corner. The test area will be to the north and west of this.

```{r}
test.tile.size <- 0.1  # degrees
test.tile.x.offset <- 0.75  # west from right edge
test.tile.y.offset <- 0.3  # north from bottom edge
```
...
ext(tmp)
gssurgo.crop <- crop(gssurgo, ext(tmp))
...
gssurgo.crop <- terra::project(gssurgo.crop, crs.utm)
sg.crop <- terra::project(sg.crop, crs.utm)
polaris.crop <- terra::project(polaris.crop, crs.utm)
```
...
```{r zlim}
values.all <- c(values(gssurgo.crop),
                values(sg.crop),
                values(polaris.crop))
...
plot(gssurgo.crop, main="gSSURGO", range=zlim)
...
stats.compare[1, ] <- c("SoilGrids250",
                        me(values(gssurgo.crop),values(sg.crop)),
                        rmse(values(gssurgo.crop),values(sg.crop)),
                        rmse.adj(values(gssurgo.crop),values(sg.crop))
                        )
stats.compare[2, ] <- c("POLARIS",
                        me(values(gssurgo.crop),values(polaris.crop)),
                        rmse(values(gssurgo.crop),values(polaris.crop)),
                        rmse.adj(values(gssurgo.crop),values(polaris.crop))
)
print(stats.compare)
```
...
sg.adj <- sg.crop + me(values(gssurgo.crop),values(sg.crop))
polaris.adj <- polaris.crop + me(values(gssurgo.crop),values(polaris.crop))
```

Recompute limits:

```{r zlim.adj}
values.all.adj <- c(values(gssurgo.crop), values(sg.adj), values(polaris.adj))
...
stats.compare <- data.frame(PSM_product = "", MD = 0, RMSD = 0, RMSD.Adjusted = 0)
stats.compare[1, ] <- c("SoilGrids250",
                        me(values(gssurgo.crop),values(sg.adj)),
                        rmse(values(gssurgo.crop),values(sg.adj)),
                        rmse.adj(values(gssurgo.crop),values(sg.adj))
                        )
stats.compare[2, ] <- c("POLARIS",
                        me(values(gssurgo.crop),values(polaris.adj)),
                        rmse(values(gssurgo.crop),values(polaris.adj)),
                        rmse.adj(values(gssurgo.crop),values(polaris.adj))
                        )
print(stats.compare)
```
...
```{r classify.raster}
gssurgo.class <- classify(gssurgo.crop, rcl=class.limits)
...
```{r vmaps.gssurgo.sg, fig.width=4, fig.height=6}
...
```{r vmaps.gssurgo.polaris, fig.width=4, fig.height=6}
par(mfrow=c(1, 2))
...
```{r vmaps.sg.polaris, fig.width=4, fig.height=6}
...
(g <- show.landscape(r.gssurgo, "gSSURGO"))
(g <- show.landscape(r.sg, "SoilGrids250"))
(g <- show.landscape(r.polaris, "POLARIS"))
```
...
names(metrics.table)[2:5] <- ls.metrics.gssurgo$metric
```
...
```{r make.sp}
gssurgo.sp <- as(raster(gssurgo.crop), "SpatialPointsDataFrame")
...
v.gssurgo <- gstat::variogram(voi ~ 1, loc = gssurgo.sp, cutoff=cutoff.init, width=500)
...
autoformat(x)
capture.output(print(x, include.rownames=FALSE), file=
                 paste0("../LaTeX_tables/compare_variograms_local_x`",
                           AOI.dir.prefix, "_", voi.depth, ".tex"))
```

...
4. Select the [maps to compare](#which); default (always included) are gNATSGO and SoilGrids250.
...
conversions <- data.frame(property=voi.list.sg, 
                 # sg=c("%%","%%","%%","pHx10","mmol(c)/kg","dg/kg","cg/cm3", "cm3/dm3"), #SG
                 gnatsgo=c(10, 10, 10, 10 , 10, 0.01*som.to.soc, 100, NA),  # gNATSGO
...
```{r hist.sg.props, fig.width=15, fig.height=15}
par(mfrow=c(3,3))
...
```{r map.sg.props, fig.width=18, fig.height=18}
par(mfrow=c(3,3))
...
```{r hist.diff.sg, fig.width=15, fig.height=15}
...
```{r plot.diff.sg, fig.width=15, fig.height=15}
par(mfrow=c(2,3))

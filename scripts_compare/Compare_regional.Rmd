...
5. Generated figures will be in directory `./figs/compare_sg/`.

6. Generated tables in \LaTeX format will be in directory `../LaTeX_tables`.

7. Generated harmonized maps will be [saved](#save) to directory `PSM_compare` under the base directory
...
```{r crs.resample}
r.gsm.sg <- terra::resample(r.gsm, r.sg)
...
```{r hist.sg.props, fig.width=14, fig.height=16}
yl <- c(0, 0.14) # standardize the density axis
par(mfrow=c(3,2))
hist(r.gsm.sg, main="GSM v0.5",
     xlim=zlim, xlab="", freq = FALSE, ylim=yl)
hist(r.sg, main="SoilGrids250",
     xlim=zlim, xlab="", freq = FALSE, ylim=yl)
if (exists("r.p.sg")) {
  hist(r.p.sg, main="POLARIS",
       xlim=zlim, xlab="", freq = FALSE, ylim=yl)
}
if (exists("r.psu.sg")) {
  hist(r.psu.sg, main="SPCG100",
       xlim=zlim, xlab="", freq = FALSE, ylim=yl)
}
if (exists("r.landgis.sg")) {
  hist(r.landgis.sg, main="LandGIS",
       xlim=zlim, xlab="", freq = FALSE, ylim=yl)
}
if (exists("r.issr8.sg")) {
  hist(r.issr8.sg, main="ISSR-800",
       xlim=zlim, xlab="", freq = FALSE, ylim=yl)
}
par(mfrow=c(1,1))
```
...
```{r hist.diff.sg, fig.width=14, fig.height=16}
yl <- c(0, 0.14) # Standardize density axis
par(mfrow=c(3,2))
hist(diff.gsm.sg, main="GSM v0.5 - SoilGrids250", xlab="",
     xlim=zlim, freq = FALSE, ylim=yl)
if (exists("r.p.sg")) {

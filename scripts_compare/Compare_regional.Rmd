...
conversions <- data.frame(property=voi.list.sg, 
                 # sg=c("%%","%%","%%","pHx10","mmol(c)/kg","dg/kg","cg/cm3"),  #SG
                 p=c(10,  10, 10,   10, NA, NA, 100),  # POLARIS -- SOM is special case
                 gsm=c(1, 1, 1, 1 ,  10, 0.01, 0.01),  # GSM
                 spcg=c(10,  10, 10, 10, NA,  1,   100),  # SPCG100USA
                 lgis=c(10,  10, 10, 1, NA, 20, 0.1),  # LandGIS
                 issr=c(10,  10, 10,  10, 10, NA,   1)  # ISSR-800
                 )
knitr::kable(
  conversions, caption = 'Conversion factors, multiply by these to match SoilGrids250',
  col.names=c("Property", "POLARIS","GlobalSoilMap", "SPCG100USA", "LandGIS", "ISSR-800"),
  booktabs = TRUE)
```
...
```{r compare.zlim}
zlim <- c(floor(min(values(r.sg)*10, values(r.issr8.sg)*10, na.rm=TRUE)),
          ceiling(max(values(r.sg)*10, values(r.issr8.sg)*10, na.rm=TRUE)))/10
if (exists("r.p.sg")) {
...
if (exists("r.psu.sg")) {
  hist(r.p.sg, main="SPCG100", xlim=zlim, xlab="")
}
if (exists("r.gsm.sg")) {
...
```{r}
summary(diff.issr8.sg <-  r.issr8.sg - r.sg)
if (exists("r.p.sg")) {
  summary(diff.issr8.p <-  r.issr8.sg - r.p.sg) }
if (exists("r.psu.sg")) {
  summary(diff.issr8.psu <-  r.issr8.sg - r.psu.sg) }
if (exists("r.gsm.sg")) {
  summary(diff.issr8.gsm <-  r.issr8.sg - r.gsm.sg) }
```
...
stats.compare[1, ] <- c("SoilGrids250",
                        me(values(r.issr8.sg),values(r.sg)),
                        rmse(values(r.issr8.sg),values(r.sg)),
                        rmse.adj(values(r.issr8.sg),values(r.sg))
                        )
if (exists("r.p.sg")) {
  stats.compare[2, ] <- c("POLARIS",
                        me(values(r.issr8.sg),values(r.p.sg)),
                        rmse(values(r.issr8.sg),values(r.p.sg)),
                        rmse.adj(values(r.issr8.sg),values(r.p.sg))
                        )
}
if (exists("r.psu.sg")) {
  stats.compare[3, ] <- c("SPCG100",
                        me(values(r.issr8.sg),values(r.psu.sg)),
                        rmse(values(r.issr8.sg),values(r.psu.sg)),
                        rmse.adj(values(r.issr8.sg),values(r.psu.sg))
                        )
}
if (exists("r.gsm.sg")) {
  stats.compare[4, ] <- c("GSM v0.5",
                        me(values(r.issr8.sg),values(r.gsm.sg)),
                        rmse(values(r.issr8.sg),values(r.gsm.sg)),
                        rmse.adj(values(r.issr8.sg),values(r.gsm.sg))
                        )
}
```

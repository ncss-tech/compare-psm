...
params:
   ...
   test.tile.x.offset: 0.54  # west from right edge
   test.tile.y.offset: 0.77  # north from bottom edge
output:
  html_document:
    fig_align: center
    fig_height: 5
...
#             main="SoilGrids250")
#
sg.class <- classify(sg.crop, rcl=class.limits)
...
```{r make_class_maps}
my.pal <- brewer.pal(n.class, "YlOrRd")
...
if (exists("r.psu")) (g <- show.landscape(r.psu, "SPCG100USA"))
if (exists("r.polaris")) (g <- show.landscape(r.polaris, "POLARIS"))
...
df.row.names <- c( "gNATSGO", "SoilGrids")
if (exists("r.psu")) {
  cove.df <- rbind(cove.df, cove.psu$signature[[1]][1,])
  df.row.names <- c(df.row.names, "SPCG100USA")
}
if (exists("r.polaris")) {
  cove.df <- rbind(cove.df, cove.polaris$signature[[1]][1,])
  df.row.names <- c(df.row.names, "POLARIS")
}
if (exists("r.gsm")) {

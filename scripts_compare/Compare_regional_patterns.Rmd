...
Test on small part, 0.2 x 0.2 degrees
...
```{r zlim.adj}
zlim.adj <- c(floor(min(values(sg.crop)*10,
                    values(gnatsgo.crop)*10,
                    ifelse(exists("gsm"), values(gsm.crop)*10, NA),
                    ifelse(exists("issr8.crop"), values(issr8.crop)*10, NA),
                    ifelse(exists("psu.crop"), values(psu.crop)*10, NA),
...
metrics.table <- data.frame(product=c("gNATSGO", "SoilGrids"),
                            rbind(ls.metrics.gnatsgo$value,
                                  ls.metrics.sg$value))
if (exists("ls.metrics.psu")) { metrics.table <- rbind(metrics.table, c("SPCG100USA", ls.metrics.psu$value)) }  
if (exists("ls.metrics.polaris")) { metrics.table <- rbind(metrics.table, c("POLARIS", ls.metrics.polaris$value)) }  
if (exists("ls.metrics.gsm")) { metrics.table <- rbind(metrics.table, c("GSM v0.5", ls.metrics.gsm$value)) }  
if (exists("ls.metrics.issr8")) { metrics.table <- rbind(metrics.table, c("ISSR-800", ls.metrics.issr8$value)) }  
if (exists("ls.metrics.landgis")) { metrics.table <- rbind(metrics.table, c("LandGIS", ls.metrics.landgis$value)) }  
...
options(xtable.floating = FALSE)
options(xtable.timestamp = "")
x <- xtable(metrics.table, row.names=FALSE, digits=rep.int(3,6))
autoformat(x)
capture.output(print(x, include.rownames=FALSE), file=
...
gnatsgo.sp <- spTransform(gnatsgo.sp, crs.utm)
...
vmeasure.compare[1,] <- c("gNATSGO", 
                       ...
                       round(vmf.gnatsgo[1,"psill"]/sum(vmf.issr8[,"psill"]), 2)
                       )
...
p1 <- plot(v.gnatsgo, ylim=ylims, model=vmf.gnatsgo, main="gNATSGO", xlab="separation (m)", pch=20)

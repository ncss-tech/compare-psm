...
values.all <- c(values(gnatsgo.crop),
                values(sg.crop), 
                ifelse(exists("gsm.crop"), values(gsm.crop), NA),
                ifelse(exists("issr8.crop"), values(issr8.crop), NA),
                ifelse(exists("psu.crop"), values(psu.crop), NA),
                ifelse(exists("landgis.crop"), values(landgis.crop), NA),
                ifelse(exists("polaris.crop"), values(polaris.crop), NA))
length(is.na(values.all))
...
zlim <- c(floor(min(values(sg.crop)*10,
                    ...
                    ifelse(exists("gsm.crop"), 
                           min(values(gsm.crop))*10, NA),
                    ifelse(exists("issr8.crop"), 
                           min(values(issr8.crop))*10, NA),
                    ifelse(exists("psu.crop"), 
                           min(values(psu.crop))*10, NA),
                    ifelse(exists("polaris.crop"),
                           min(values(polaris.crop))*10, NA),
                    ifelse(exists("landgis.crop"),
                           min(values(landgis.crop))*10, NA),
                    na.rm=TRUE)),
          ceiling(max(values(sg.crop)*10,
                      values(gnatsgo.crop)*10,
                      ifelse(exists("gsm.crop"), 
                             max(values(gsm.crop))*10, NA),
                      ifelse(exists("issr8.crop"), 
                             max(values(issr8.crop))*10,  NA),
                      ifelse(exists("psu.crop"),
                             max(values(psu.crop))*10, NA),
                      ifelse(exists("polaris.crop"), 
                             max(values(polaris.crop))*10, NA),
                      ifelse(exists("landgis.crop"),
                             max(values(landgis.crop))*10, NA),
                      na.rm=TRUE)))/10
print(zlim)
```
...
if (exists("landgis.adj")) stats.compare[4, ] <- c("POLARIS",
...
if (exists("issr8.adj")) stats.compare[5, ] <- c("POLARIS",
...
```{r classify.setup}
(cut.names <- cut(zlim.adj, breaks=c(zlim.adj[1], cuts, zlim.adj[2]),
...
#
tmpf <- tempfile()
terra::writeVector(gnatsgo.poly, tmpf, overwrite=TRUE)
gnatsgo.sf <- st_read(tmpf, quiet = TRUE); unlink(tmpf)
...
if (exists("gsm.class")) { 
  tmpf <- tempfile()
  terra::writeVector(gsm.poly, tmpf, overwrite=TRUE)
  gsm.sf <- st_read(tmpf, quiet = TRUE); unlink(tmpf)
  colnames(gsm.sf)[1] <- "class"
}
#
if (exists("psu.class")) {
  tmpf <- tempfile()
  terra::writeVector(psu.poly, tmpf, overwrite=TRUE)
  psu.sf <- st_read(tmpf, quiet = TRUE); unlink(tmpf)
...
if (exists("issr8.class")) { 
  ...
  issr8.sf <- st_read(tmpf, quiet = TRUE); unlink(tmpf)
  colnames(issr8.sf)[1] <- "class"
}
if (exists("landgis.class")) {
  tmpf <- tempfile()
  terra::writeVector(landgis.poly, tmpf, overwrite=TRUE)
  landgis.sf <- st_read(tmpf, quiet = TRUE); unlink(tmpf)
...
```{r vmaps.sg.psu, fig.width=4, fig.height=6}
if (exists("psu.sf.v")) {
...
p1 <- plot(v.sg, ylim=ylims, model=vmf.sg, main="SoilGrids250", xlab="separation (m)", pch=20)
...
if(exists("vmf.psu")) {
  ...
  p2 <- plot(v.psu, ylim=ylims, model=vmf.psu, main="SPCG100USA", xlab="separation (m)", pch=20)
  print(p2, split=c(c, r, n.figs.col, n.figs.row), more=T); c <- c + 1
} 
if(exists("vmf.polaris")) {
  pos <- rc.pos(r, c); r <- pos[1]; c <- pos[2]
  p3 <- plot(v.polaris, ylim=ylims, model=vmf.polaris, main="POLARIS", xlab="separation (m)", pch=20)

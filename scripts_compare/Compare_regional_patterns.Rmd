...
```{r check.landscape}
check_landscape(r.sg, verbose=TRUE)
```
...
ls.metrics.polaris <- ifelse(exists("r.polaris"), calculate_lsm(r.polaris, what=lst),
                             rep(NA, 4))
ls.metrics.gsm <- ifelse(exists("r.gsm"), calculate_lsm(r.gsm, what=lst),
                           rep(NA, 4))
...
```{r}
metrics.table <- data.frame(product=c("gNATSGO", "SoilGrids", "SPCG100USA", "POLARIS", "GSM v0.5", 
                                      "ISSR8", "LandGIS"),
                            rbind(ls.metrics.gnatsgo$value,
                                  ls.metrics.sg$value,
                                  ls.metrics.psu$value,
                                  ls.metrics.polaris$value,
                                  ls.metrics.gsm$value,

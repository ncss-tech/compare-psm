...
```{r}
(gnatsgo <- rast(paste0(base.dir, aoi, "/gnatsgo_tile_250_", voi.depth, ".tif")))
names(gnatsgo) <- "voi"
(sg <- rast(paste0(base.dir, aoi, "/sg_tile_250_", voi.depth, ".tif")))
names(sg) <- "voi"
if ("GSM v0.5" %in% products) {
  file.name <- paste0(base.dir, aoi, "/gsm_tile_250_", voi.depth, ".tif")
  if (file.exists(file.name)) { (gsm <- rast(file.name)); names(gsm) <- "voi" }
}
...
zlim <- c(floor(min(values(sg.crop)*10,
                    values(gnatsgo.crop)*10,
                    ifelse(exists("gsm"), values(gsm.crop)*10, NA),
...
zlim.adj <- c(floor(min(values(sg.adj)*10,
                    values(gnatsgo.crop)*10,
                    ifelse(exists("gsm"), values(gsm.adj)*10, NA),
...
```{r reset.adj}
# gnatsgo was the basis
sg.adj <- sg.crop
if (exists("gsm.crop")) gsm.adj <- gsm.crop
...
values.all <- c(values(gnatsgo.crop),
                values(sg.adj), 
                ifelse(exists("gsm.adj"), values(gsm.adj), NA),
...
#
if (exists("gsm.adj")) {
  gsm.class <- classify(gsm.adj, rcl=class.limits)
...
```{r show.classified, fig.width=18, fig.height=12}
par(mfrow=c(3,3))
.l <- range(values(gnatsgo.class), na.rm=TRUE)
terra::plot(gnatsgo.class,
            col=color.ramp[.l[1]:.l[2]], type="classes",
...
```{r polygonize}
gnatsgo.poly <- terra::as.polygons(gnatsgo.class, dissolve=TRUE, trunc=FALSE)
...
#
tmpf1 <- tempfile()
terra::writeVector(gnatsgo.poly, tmpf1, overwrite=TRUE)
gnatsgo.sf <- st_read(tmpf1); unlink(tmpf1)
names(gnatsgo.sf)[1] <- "class"
#
tmpf2 <- tempfile()
...
if (exists("gsm.class")) { 
  tmpf5 <- tempfile()
  terra::writeVector(gsm.poly, tmpf5, overwrite=TRUE)
  gsm.sf <- st_read(tmpf5); unlink(tmpf5)
  colnames(gsm.sf)[1] <- "class"
}
#
if (exists("psu.class")) {
  tmpf3 <- tempfile()
...
```{r class_maps, fig.width=5, fig.height=7}
print(g0)
print(g1)
if (exists("g2")) { print(g2) }
if (exists("g3")) { print(g3) }
if (exists("g4")) { print(g4) }
if (exists("g5")) { print(g5) }
if (exists("g6")) { print(g6) }
```
...
```{r vmaps.gnatsgo.sg, fig.width=5, fig.height=7}
  ## produced maps -- the homogeneity of the regions.gnatsgo.sg
terra::plot(regions.gnatsgo.sg$map1["rih"], main = "Homogeneity -- SG250 vs. gNATSGO")
terra::plot(regions.gnatsgo.sg$map2["rih"], main = "Completeness -- SG250 vs. gNATSGO")
```

### gNATSGO vs. SPCG100USA

```{r vmaps.gnatsgo.psu, fig.width=5, fig.height=7}
if (exists("psu.sf.v")) {
  ...
  terra::plot(regions.gnatsgo.psu$map1["rih"], main = "Homogeneity -- SPCG100USA vs. gNATSGO")
  terra::plot(regions.gnatsgo.psu$map2["rih"], main = "Completeness -- SPCG100USA vs. gNATSGO")
}
```
...
```{r vmaps.gnatsgo.polaris, fig.width=5, fig.height=7}
...
terra::plot(regions.gnatsgo.psu$map1["rih"], main = "Homogeneity -- POLARIS vs. gNATSGO")
terra::plot(regions.gnatsgo.psu$map2["rih"], main = "Completeness -- POLARIS vs. gNATSGO")
```

### SoilGrids250 vs. SPCG100USA

```{r vmaps.sg.psu, fig.width=5, fig.height=7}
if (exists("psu.sf.v")) {
...
if (exists("regions.gnatsgo.psu")) {
  vmeasure.compare[i <- i+1,] <- c("gNATSGO vs. SPCG100USA", 
...
if (exists("regions.gnatsgo.polaris")) {
  vmeasure.compare[i <- i+1,] <- c("gNATSGO vs. POLARIS", 
...
(g1 <- show.landscape(r.gnatsgo, "gNATSGO"))
...
if (exists("r.gsm")) (g4 <- show.landscape(r.gsm, "GSM v0.5"))
...
```{r}
metrics.table <- data.frame(product=c("gNATSGO", "SoilGrids"),
...
if (exists("ls.metrics.psu")) { metrics.table <- rbind(metrics.table, 
                                                       c("SPCG100USA", round(ls.metrics.psu$value, 3))) }  
if (exists("ls.metrics.polaris")) { metrics.table <- rbind(metrics.table, 
                                                           c("POLARIS", round(ls.metrics.polaris$value, 3))) }  
...
names(metrics.table)[2:5] <- ls.metrics.gnatsgo$metric
```
...
if (exists("r.gsm")) {
  gsm.sp <- as(raster(gsm.adj), "SpatialPointsDataFrame") }
if (exists("r.psu")) {
...
```{r}
range.init <- 2000  # m 
cutoff.init <- range.init*2.5  # m
```

Compute and model variograms. Note the estimated sill is the maximum $\gamma$ in the empirical variogram.

Model the variograms with the exponential model; the range parameter is 1/3 of the effective range.
...
v.gnatsgo <- gstat::variogram(voi ~ 1, loc = gnatsgo.sp, cutoff=cutoff.init, width=500)
vm.gnatsgo <- vgm(max(v.gnatsgo$gamma), "Exp", range.init, 0)
...
```{r plot.vgms, fig.width=5, fig.height=5}
ylims <- c(0, max(v.gnatsgo$gamma, v.sg$gamma, 
                  ifelse(exists("v.gsm"), v.gsm$gamma, 0),
                  ifelse(exists("v.psu"), v.psu$gamma, 0),
                  ifelse(exists("v.polaris"), v.polaris$gamma, 0),
                  ifelse(exists("v.issr8"), v.issr8$gamma, 0),
                  ifelse(exists("v.landgis"), v.landgis$gamma, 0))*1.1)
p <- plot(v.gnatsgo, ylim=ylims, model=vmf.gnatsgo, main="gNATSGO", xlab="separation (m)", pch=20)
print(p)
p <- plot(v.sg, ylim=ylims, model=vmf.sg, main="SoilGrids250", xlab="separation (m)", pch=20)
print(p)
if(exists("vmf.psu")) {
  p <- plot(v.psu, ylim=ylims, model=vmf.psu, main="SPCG100USA", xlab="separation (m)", pch=20)
  print(p)
} 
if(exists("vmf.polaris")) {
  p <- plot(v.polaris, ylim=ylims, model=vmf.polaris, main="POLARIS", xlab="separation (m)", pch=20)
  print(p)
} 
if(exists("vmf.gsm")) {
  p <- plot(v.gsm, ylim=ylims, model=vmf.gsm, main="GSM v0.5", xlab="separation (m)", pch=20)
  print(p)
} 
if(exists("vmf.issr8")) {
  p <- plot(v.issr8, ylim=ylims, model=vmf.issr8, main="ISSR-800", xlab="separation (m)", pch=20)
  print(p)
} 
if(exists("vmf.landgis")) {
  p <- plot(v.landgis, ylim=ylims, model=vmf.landgis, main="LandGIS", xlab="separation (m)", pch=20)
  print(p)
} 
```

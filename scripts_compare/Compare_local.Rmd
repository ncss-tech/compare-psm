...
```{r base.dir.export}
base.dir.export <- paste0(base.dir, "Compare_30m")
```
...
conversions <- data.frame(property=voi.list.sg, 
                 # sg=c("%%","%%","%%","pHx10","mmol(c)/kg","dg/kg","cg/cm3", "cm3/dm3"), #SG
                 gssurgo=c(10, 10, 10, 10 , 10, 0.01*som.to.soc, 100, NA),  # gSSURGO
...
Resample products into POLARIS WGS84 30m nominal pixels, using cubic interpolation.
...
```{r crs.project}
r.gssurgo.p <- terra::project(r.gssurgo, r.p, method="cubic") 
plot(r.gssurgo.p)
```
...
plot(r.gssurgo.p, main="gSSURGO")
plot(r.p, main="POLARIS")
plot(r.sg.p, main="SoilGrids250")
par(mfrow=c(1, 1))
```
...
yl <- max(max.dens(r.gssurgo.p), max.dens(r.sg))
...
```{r}
diff.gssurgo.p <- r.gssurgo.p - r.sg
if (exists("r.gsm.sg")) { diff.gssurgo.gsm <- r.gssurgo.p - r.gsm.sg }
if (exists("r.p.sg")) {  diff.gssurgo.p <-  r.gssurgo.p - r.p.sg }
if (exists("r.psu.sg")) {  diff.gssurgo.psu <-  r.gssurgo.p - r.psu.sg }
if (exists("r.landgis.sg")) {  diff.gssurgo.landgis <-  r.gssurgo.p - r.landgis.sg }
if (exists("r.issr8.sg")) { diff.gssurgo.issr8 <-  r.gssurgo.p - r.issr8.sg }
```
...
stats.compare[1, ] <- c("SoilGrids250",
                        me(values(r.gssurgo.p),values(r.sg)),
                        rmse(values(r.gssurgo.p),values(r.sg)),
                        rmse.adj(values(r.gssurgo.p),values(r.sg))
); i <- 2
if (exists("r.gsm.sg")) {
  stats.compare[i, ] <- c("GSM v0.5",
                        me(values(r.gssurgo.p),values(r.gsm.sg)),
                        rmse(values(r.gssurgo.p),values(r.gsm.sg)),
                        rmse.adj(values(r.gssurgo.p),values(r.gsm.sg))
                        ); i <- i + 1
}
if (exists("r.p.sg")) {
  stats.compare[i, ] <- c("POLARIS",
                        me(values(r.gssurgo.p),values(r.p.sg)),
                        rmse(values(r.gssurgo.p),values(r.p.sg)),
                        rmse.adj(values(r.gssurgo.p),values(r.p.sg))
                        ); i <- i + 1
}
if (exists("r.psu.sg")) {
  stats.compare[i, ] <- c("SPCG100",
                        me(values(r.gssurgo.p),values(r.psu.sg)),
                        rmse(values(r.gssurgo.p),values(r.psu.sg)),
                        rmse.adj(values(r.gssurgo.p),values(r.psu.sg))
                        ); i <- i + 1
}
if (exists("r.landgis.sg")) {
  stats.compare[i, ] <- c("LandGIS",
                        me(values(r.gssurgo.p),values(r.landgis.sg)),
                        rmse(values(r.gssurgo.p),values(r.landgis.sg)),
                        rmse.adj(values(r.gssurgo.p),values(r.landgis.sg))
                        ); i <- i + 1
}
if (exists("r.issr8.sg")) {
  stats.compare[i, ] <- c("ISSR-800",
                        me(values(r.gssurgo.p),values(r.issr8.sg)),
                        rmse(values(r.gssurgo.p),values(r.issr8.sg)),
                        rmse.adj(values(r.gssurgo.p),values(r.issr8.sg))
                        )
}
```
...
if (exists("diff.gssurgo.gsm")) {
  zlim <- c(floor(min(zlim[1]*10, values(diff.gssurgo.gsm)*10, na.rm=TRUE)),
            ceiling(max(zlim[2]*10, values(diff.gssurgo.gsm)*10, na.rm=TRUE)))/10
}
if (exists("diff.gssurgo.p")) {
  zlim <- c(floor(min(zlim[1]*10, values(diff.gssurgo.p)*10, na.rm=TRUE)),
            ceiling(max(zlim[2]*10, values(diff.gssurgo.p)*10, na.rm=TRUE)))/10
}
if (exists("diff.gssurgo.psu")) {
  zlim <- c(floor(min(zlim[1]*10, values(diff.gssurgo.psu)*10, na.rm=TRUE)),
            ceiling(max(zlim[2]*10, values(diff.gssurgo.psu)*10, na.rm=TRUE)))/10
}
if (exists("diff.gssurgo.landgis")) {
  zlim <- c(floor(min(zlim[1]*10, values(diff.gssurgo.landgis)*10, na.rm=TRUE)),
            ceiling(max(zlim[2]*10, values(diff.gssurgo.landgis)*10, na.rm=TRUE)))/10
}
if (exists("diff.gssurgo.issr8")) {
  zlim <- c(floor(min(zlim[1]*10, values(diff.gssurgo.issr8)*10, na.rm=TRUE)),
            ceiling(max(zlim[2]*10, values(diff.gssurgo.issr8)*10, na.rm=TRUE)))/10
}
```
...
yl <- max(max.dens(diff.gssurgo.p))
if (exists("r.gsm.sg")) { yl <- max(yl, max.dens(diff.gssurgo.gsm))}
if (exists("r.p.sg")) { yl <- max(yl, max.dens(diff.gssurgo.p))}
if (exists("r.psu.sg")) { yl <- max(yl, max.dens(diff.gssurgo.psu))}
if (exists("r.landgis.sg")) { yl <- max(yl, max.dens(diff.gssurgo.landgis))}
if (exists("r.issr8.sg")) { yl <- max(yl, max.dens(diff.gssurgo.issr8))}
```
...
hist(diff.gssurgo.p, main="gSSURGO - SoilGrids250", xlab="",
     xlim=zlim,  breaks=24, freq = FALSE, ylim=yl)
if (exists("r.gsm.sg")) { 
  hist(diff.gssurgo.gsm, main="gSSURGO - GSM v0.5", xlab="",
     xlim=zlim,  breaks=24, freq = FALSE, ylim=yl)
}
if (exists("r.p.sg")) {
...
if (exists("r.psu.sg")) {
  hist(diff.gssurgo.psu, main="gSSURGO - SPCG100USA", xlab="",
     xlim=zlim,  breaks=24, freq = FALSE, ylim=yl)
}
if (exists("r.landgis.sg")) {
  hist(diff.gssurgo.landgis, main="gSSURGO - LandGIS", xlab="",
     xlim=zlim,  breaks=24, freq = FALSE, ylim=yl)
}
if (exists("r.issr8.sg")) {
  hist(diff.gssurgo.p, main="gSSURGO - ISSR-800", xlab="",
     xlim=zlim,  breaks=24, freq = FALSE, ylim=yl)
}
par(mfrow=c(1,1))
```
...
par(mfrow=c(n.figs.row.diff, n.figs.col.diff))
  terra::plot(diff.gssurgo.p, main="Difference gSSURGO - SoilGrids250",
            range=zlim, col=bpy.colors(64))
if (exists("r.gsm.sg")) { 
  terra::plot(diff.gssurgo.gsm, main="Difference gSSURGO - GSM v0.5",
            range=zlim, col=bpy.colors(64))
}
if (exists("diff.gssurgo.p")) {
...
if (exists("diff.gssurgo.psu")) {
  terra::plot(diff.gssurgo.psu, main="Difference gSSURGO - SPCG100USA",
            range=zlim, col=bpy.colors(64))
}
if (exists("diff.gssurgo.landgis")) {
  terra::plot(diff.gssurgo.landgis, main="Difference gSSURGO - LandGIS",
            range=zlim, col=bpy.colors(64))
}
if (exists("diff.gssurgo.issr8")) {
  terra::plot(diff.gssurgo.issr8, main="Difference gSSURGO - ISSR-800",
            range=zlim, col=bpy.colors(64))
}
par(mfrow=c(1,1))
```
...
dest.name <- paste0(dest.dir.save,"/gssurgo_tile_250_",  voi.depth.sg, ".tif")

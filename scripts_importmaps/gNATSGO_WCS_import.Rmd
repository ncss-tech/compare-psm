...
```{r wcs.aoi}
(wcs.aoi <- c(tile.ulc[1],tile.lrc[2], tile.lrc[1], tile.ulc[2]))
```
...
For some tiles, some of the map units have incorrect horizination.

Make an ID field for reshaping; this is the same for all components:

```{r}
mu.attr$variable.id <- sprintf("%s%s%s%s", mu.attr$variable, "_", mu.attr$top, mu.attr$bottom)
```
...
## Weight the property by the component proportions
...
Call the weight function for each map unit and add the result to the dataframe of map unit IDs:
...
for (i in 1:length(mu.site.split)) {
  ...
  mu.list[i, result.field] <- mean.wt
}
head(mu.list)
```

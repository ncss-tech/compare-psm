---
title: "Importing USA 100m grids"
author: "D G Rossiter"
date: "`r Sys.Date()`"
output:
  html_document:
...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, fig.align = 'center', fig.path = './figs/diffs_spcgusa_sg_polaris/')
...
This documents has two parts. The first is a long step-by-step explanation with a single $1 \times 1^\circ$ Area of Interest (AOI) which can be adjusted manually [in this section](#aoi), property, and depth which can be adjusted manually [in this section](#voi). The second is a self-contained [function](#function)  to carry out the analysis for a user-selected set of properties, depths and quantiles for a single AOI and save all the results in the file system; this is [here](#compare). This set of maps can then be used to compare prediction intervals.`

When compiling this document with `knitr`, sample calls to the function are suppressed with the `eval=FALSE` markdown tag.
...
3. [Select a property](#prop) and [select a depth slice](#depth).

4. [Select an Area of Interest](#aoi).
...
# Set the Area of Interest (AOI) {#aoi}

Specify the two corners using WGS84 geographic coordinates, and the size of the tile. If comparing with POLARIS these should be $1 \times 1^\circ$.
...
Make a `terra` `SpatExtent` object from the bounding box:

```{r plot.properties.crop, fig.width=10, fig.height=5}

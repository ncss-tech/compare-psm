...
```{r}
voi <- "soc"   # variable of interest, SoilGrids name
depth <- "0-5cm" # "0-5", "5-15", "15-30", "30-60", "60-100", "100-200"
quantile.sg <- "mean"  # "Q0.05", "Q0.5" [not 0.50 !], "Q0.95", "mean"
(voi_layer <- paste(voi, depth, quantile.sg, sep="_")) # layer of interest 
```

# Define Area of Interest (AOI) {#aoi}

We use a $1 \times 1^\circ$ tile, because that is how POLARIS data is served.
Specify the lower-right corner, then compute the upper-right corner $1^\circ$ west and north.

Specify the four corners using WGS84 geographic coordinates; this will then transform to a bounding box in any CRS.

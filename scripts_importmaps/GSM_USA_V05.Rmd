...
* [Grid](https://edcftp.cr.usgs.gov/project/soil/GlobalSoilMap/mukeyint_SaS_3as.7z)
 https://edcftp.cr.usgs.gov/project/soil/GlobalSoilMap/mukeyint_SaS_3as.7z

Download and then uncompress into a directory named `mukeyint_SaS_3as`, under `base.dir.gsm05.import` (see above). This has a GeoTIFF and related metadata files; total size 9.8 GB.
...
* [Metadata](https://edcftp.cr.usgs.gov/project/soil/GlobalSoilMap/T_GSM_20180430162220_metadata.xml) written by Norman Bliss. 
...
library(sf)
library(rgdal)
library(raster)   # `terra` has some problems with clipping
library(tidyverse)
``` 
...
voi.list.gsm <- c("claytotal_r_g_kg", "silttotal_r_g_kg", "sandtotal_r_g_kg", "ph1to1h2o_r_ions_pHx10", "ecec_r_cmolc_kg", "soc_r_mr_g_gF")
```
...
Get the representative values for the selected property and replace NoData values used in the database with R `NA`. The NoData values vary by data type. Also include the map unit key, for joining with the grid.
...
*Specify the two corners using WGS84 geographic coordinates and the tile size*.

```{r tile.1}
## Lower-right corner, in integer degrees
#
# tile.lrc <- c(-76, 42) # lower-right corner: central NY
tile.lrc <- c(-77, 35) # coastal plain NC
#
...
```{r link}
r.attr <- subs(r.crop, db.attr,
               by=1,
               which=match(voi.depth.name, names(db.attr)))
plot(r.attr)
summary(r.attr)
```
...
```{r}
f <- raster::writeRaster(r.attr, file=paste0(dest.dir, "/GSM_",
                                              voi.depth.name, ".tif"),

...
The aim of this script is to facilitate evaluation of [gNATSGO](https://www.nrcs.usda.gov/wps/portal/nrcs/detail/soils/survey/geo/?cid=nrcseprd1464625).
...
This script must follow the import of the tile covering the Area of Interest (AOI), property, depth and quantile. The import script is `..scripts_importmaps/gNATSGO_WCS_import.Rmd`. The tile is placed by that script at the base directory ([see below](#dir)), in a subdirectory structure using the AOI.
...
(voi.depth.name <- paste0(voi.name, "_", depth.list.gnatsgo[params$depth.n]))
```
...
# Get the imported gNATSGO tile
  
```{r get.tile}
src.dir.gnatsgo <-  paste0(base.dir.gnatsgo, "/", AOI.dir.prefix)
```
...
```{r get-sg}
file.name <- paste0(src.dir.gnatsgo, "/", voi.depth.name, ".tif")
...
r.gnatsgo <- crop(r.gnatsgo, bb.sub)
print(r.gnatsgo)
plot(r.gnatsgo, col=topo.colors(64))
```

Convert to WGS84 long/lat as required by Google Earth:

```{r}
r.gnatsgo.4326 <- terra::project(r.gnatsgo, 
                                 "+proj=longlat +datum=WGS84",
                                 method="cubic")
# correct -3.4e+38 'NA' values
v <- values(r.gnatsgo.4326)
values(r.gnatsgo.4326) <- ifelse((v < 0), NA, v)
plot(r.gnatsgo.4326)
r.gnatsgo.4326
```
...
```{r create.spdf}
class(r.gnatsgo.4326)
tmp <- raster::raster(r.gnatsgo.4326)
class(tmp)
...
```{r}
(filename.png <- paste0(src.dir.gnatsgo,"/", voi.depth.name, '.png'))
...
```{r}
(filename.kml <- paste0(src.dir.gnatsgo,"/", voi.depth.name, '.kml'))

...
```{r}
tile.lrc <- c(-76.1, 42.2) # lower-right corner: central NY
d.long <- 0.4; d.lat <- 0.4
```
...
## WCS access to SoilGrids250

We set other variables necessary for the WCS call for all kinds of requests

```{r}
wcs_path_base <- "https://maps.isric.org/mapserv?map=/map/" # Path to the WCS. See maps.isric.org
wcs_service <- "SERVICE=WCS"
wcs_version <- "VERSION=2.0.1"
# Add the variable of interest to the path of the URL
...
This will be used with the `gdal` utility after being saved to disk.
The `datasetname` is an XML file that describes the dataset and the operation, here its description, which allows `gdal_translate` to find and download it.
